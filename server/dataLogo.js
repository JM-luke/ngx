...
function updateLogo() {
  ...
  const lastDay = moment(logoPositions[0].date, 'DD-MM-YYYY').add(1, 'days');
  let open;
  let close;

  if (counter % 2 === 0) {
    open = logoPositions[0].open + diff;
    close = logoPositions[0].close + diff;
  } else {
    open = Math.abs(logoPositions[0].open - diff);
    close = Math.abs(logoPositions[0].close - diff);
  }

  logoPositions.unshift({
